...
if "drone_process" not in st.session_state:
    st.session_state["drone_process"] = None
...
IMGSZ = st.sidebar.selectbox("Inference image size (imgsz)", [320, 416, 640, 960], index=3)
...
# Mode selector
mode = st.radio(
    f"{selected_model_name} mode",
    ["Realtime", "Drone Mode", "Video Demo"],
    index=0,
    horizontal=True
)

# ---- Reset button for Realtime & Drone Mode ----
if mode in ["Realtime", "Drone Mode"]:
    if st.sidebar.button("🔄 Reset Counts", key=f"reset_live_{selected_model_name}"):
        MODEL_STATE["current_count"] = 0
        MODEL_STATE["cumulative_total"] = 0
        MODEL_STATE["prev_max_seen"] = 0

        cur_card_ph.markdown(
            card_html("CURRENT COUNT", 0, "#2b6cb0"),
            unsafe_allow_html=True
        )
        cum_card_ph.markdown(
            card_html("CUMULATIVE COUNT", 0, "#dd6b20"),
            unsafe_allow_html=True
        )
...
class RTMPFrameGrabber:
    def __init__(self, url):
        self.cap = cv2.VideoCapture(
            url + "?fflags=nobuffer&flags=low_delay&rtmp_live=live",
            cv2.CAP_FFMPEG
        )

        # Force minimal internal buffering
...
# ----------------- DRONE MODE -----------------
    ...
    class DroneRTMPProcessor(VideoProcessorBase):
        def __init__(self, shared_state, device, imgsz, conf_thresh):
            self.processor = GenericProcessor(shared_state, device, imgsz, conf_thresh)
            self.grabber = RTMPFrameGrabber(RTMP_DRONE_URL)
        ...
        def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
            img = self.grabber.get_latest()

            if img is None:
                img = frame.to_ndarray(format="bgr24")

            fake_frame = av.VideoFrame.from_ndarray(img, format="bgr24")
            return self.processor.recv(fake_frame)


    with right_col:
        st.info("🚁 Drone Mode — RTMP Stream + Realtime Models")

        webrtc_ctx = webrtc_streamer(
            key=f"drone-{selected_model_name}",
            mode=WebRtcMode.SENDRECV,
            video_processor_factory=lambda: DroneRTMPProcessor(
                MODEL_STATE, DEVICE, IMGSZ, CONF_THRESH
            ),
    ...
    # SAME dashboard update loop as Realtime
    try:
        time.sleep(0.2)
        while True:
            current = int(MODEL_STATE.get("current_count", 0))
            ...
            cur_card_ph.markdown(
                card_html("CURRENT COUNT", current, "#2b6cb0"),
                unsafe_allow_html=True
            )
            cum_card_ph.markdown(
                card_html("CUMULATIVE COUNT", cumulative, "#dd6b20"),
                unsafe_allow_html=True
            )
            debug_ph.text(
                f"Device: {DEVICE}  |  imgsz: {IMGSZ}  |  conf: {CONF_THRESH:.2f}"
            )

            time.sleep(0.2)

    except Exception as e:
        st.error(f"Drone dashboard loop error: {e}")
...
st.markdown("---")
if st.button("Reset counts", key=f"reset_bottom_{selected_model_name}"):
    MODEL_STATE["cumulative_total"] = 0
    MODEL_STATE["prev_max_seen"] = 0
    MODEL_STATE["current_count"] = 0
    cur_card_ph.markdown(card_html("CURRENT COUNT", MODEL_STATE["current_count"], "#2b6cb0"), unsafe_allow_html=True)
